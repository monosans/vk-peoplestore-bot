...
def upgrade_slave(slave_id: int, price: int) -> None:
    ...
    while price <= UPGRADE_LIMIT:
        ...
        while not slave:
            sleep_delay()
            slave = client.user(slave_id)
        price = slave["user"]["price"]


def buy_target_slaves(target_id: int) -> None:
    """Перекупка рабов у target_id при BUY_SLAVES_MODE = 2 или 3."""
    have_bought = False
    user = client.user(target_id)
    if user:
        for slave in user["slaves"]:
            price = slave["price"]
            if slave["fetter_to"] < time() and MIN_PRICE <= price <= MAX_PRICE:
                buy_slave_fetter_upgrade(slave["id"], price, target_id)
                have_bought = True
                sleep_delay()
    if not have_bought:
        sleep_delay()
...
Доход в минуту: {me['slaves_profit_per_min']}"""
        ...
        if UPGRADE_SLAVES == 1:
            upgrade_slave(slave_id, buy_slave["slave"]["price"])
        if BUY_FETTERS == 1 and client.fetter_slave(slave_id):
...
def buy_top_users_slaves() -> None:
    ...
    TOP_EXCLUDE.append(start["user"]["id"])
    while True:
...
def hunt_ids() -> None:
    ...
    while True:
        for user_id in BUY_IDS:
            ...
            if user:
                user = user["user"]
                if user["fetter_to"] < time():
                    master_id = user["master_id"]
                    price = user["price"]
                    if master_id == my_id:
                        if BUY_FETTERS == 1 and client.fetter_slave(user_id):
    ...
    while True:
        ...
        for slave in slaves:
            ...
            if (
                ...
                slave_id = slave["id"]
                if client.fetter_slave(slave_id):
